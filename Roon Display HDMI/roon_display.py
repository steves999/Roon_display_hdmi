...
# Touch event flag — set when touch requests immediate text redisplay
_touch_show_text = False

def on_touch_event(event, data=None):
    """Handle touch events from touch_handler."""
    global _touch_show_text
    if event == "toggle_text" and data == True:
        _touch_show_text = True
    elif event in ("volume_up", "volume_down", "play_pause", "next", "previous"):
        pass  # handled directly by touch_handler via Roon API
...
def draw_touch_hints(canvas, alpha=40):
    """Draw subtle touch zone indicators at screen edges."""
    overlay = Image.new('RGBA', canvas.size, (0, 0, 0, 0))
    draw = ImageDraw.Draw(overlay)
    # Left vol- zone
    draw.rectangle([0, 0, W//3, H], fill=(255, 255, 255, alpha//3))
    # Right vol+ zone
    draw.rectangle([W*2//3, 0, W, H], fill=(255, 255, 255, alpha//3))
    # Small icons
    font = ImageFont.truetype(FONT_REG, 48)
    draw.text((W//6, H//2 - 30), "−", font=font, fill=(255, 255, 255, 120), anchor="mm")
    draw.text((W*5//6, H//2 - 30), "+", font=font, fill=(255, 255, 255, 120), anchor="mm")
    return Image.alpha_composite(canvas.convert('RGBA'), overlay).convert('RGB')
...
def display_roon(base, artist, album, track, seek_pos, length, radio=False, track_id=None):
    ...
    touch_handler.set_show_text(True)
    ...
    # Text hold — show text, check for touch toggle
    ...
    # After text hold — show clean art with volume circle, but allow touch to re-show text
    while True:
        zone_check = get_zone()
        if not zone_check or zone_check["state"] != "playing":
            break
        ...
        if _touch_show_text:
            ...
            touch_handler.set_show_text(True)
            overlay = make_text_overlay(artist, album, track, source='roon')
            # Show text for text_hold seconds then return to clean
            for _ in range(cfg["text_hold"]):
...
# Start touch handler — zone/output IDs filled in once first zone is found
...
while True:
    ...
    if zone and zone["state"] == "playing" and "now_playing" in zone:
        # Update touch handler with current zone/output IDs
        zone_id   = zone.get("zone_id")
        output_id = zone["outputs"][0].get("output_id") if zone.get("outputs") else None
        touch_handler.configure(cfg["bridge"], zone_id, output_id)
